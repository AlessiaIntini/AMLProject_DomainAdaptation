...
def one_hot_it(label, label_info):
	...
	for index, info in enumerate(label_info):
		...
		#np.concatenate((semantic_map,class_map),axis=-1)
		#semantic_map = np.stack(class_map, axis=-1)
		
	#print("semantic_map", semantic_map)	
	return semantic_map

def one_hot_it_C(label, label_info):
	# return semantic_map -> [H, W]
	semantic_map = np.zeros(label.shape[1:4])
	for index, info in enumerate(label_info):
